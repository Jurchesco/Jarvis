...
Product snapshot (2026-09-22): Home = Freestyle **or** catalog plan (`/plans`, D018). Set logging = **batch or per-set** (D016). Stats = muscle set-volume (D019 §3). Rest timer overlay after save (D019). Coach = Hermes; Gem = backup (D017). Next backlog: JSON backup / session UX polish.
